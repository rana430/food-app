...
const Amessage = (probs) => {
  ...
  return (
    <>
      {probs.msg.didIsendIt ? (
        <div className="userMsgStyle">
          {probs.msg.text.length <= 600 ? (
            <div>{probs.msg.text}</div>
          ) : !showAllLetters ? (
            <div>
              {probs.msg.text.slice(0, 600)}
              <span
                style={{ color: "blue" }}
                onClick={() => {
                  setshowAllLetters(1 - showAllLetters);
                }}
              >
                {" "}
                &nbsp; read more...
